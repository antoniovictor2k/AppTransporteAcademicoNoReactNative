  ...
  async function sendFormulario(id) {
    try {
      ...
      const response = await fetch(
        `https://back-end-transporte-academico-ifjfoi6st-antoniovictor2k.vercel.app/dados/${id}`,
        {
          method: "GET",
          headers: {
      ...
      if (json.imgUrl) {
        ...
        setImagemUrl(
          "https://t2.gstatic.com/licensed-image?q=tbn:ANd9GcQKp5ZsMFTZZrNFb_p2mduiKPipCDh7meawj_zla0mhOHNi_07O3TcMv-q-H-HBvH2G"
        ); // ou defina uma URL padrão ou uma imagem de fallback
      }
    } catch (error) {
      ...
      <View>
        ...
        <View style={styles.links}>
          ...
          <Text style={styles.link}>
            <Text onPress={() => navigation.navigate("TermosDeUso")}>
              Termos de uso
            </Text>{" "}
            e de{" "}
            <Text onPress={() => navigation.navigate("TermosDePrivacidade")}>
              Privacidade
            </Text>
          </Text>
        </View>
    ...
    <View style={{ flexDirection: "row", alignItems: "center" }}>
      <TouchableOpacity onPress={toggleDrawer}>
        <Icon
          ...
          color="#fff"
          backgroundColor="#000"
          style={{ marginLeft: 15, borderRadius: 28 }}
        />
      </TouchableOpacity>
    </View>
  );
    ...
    >
      <Drawer.Screen
        options={{
          ...
          drawerLabel: ({ focused }) => (
            <Text style={styles.optionsTextos}>Mapa</Text>
          ),
          drawerIcon: ({ focused }) => (
            <Icon style={styles.optionsIcones} name="map" />
          ),
        }}
        name="Mapa"
        component={TelaPrincipal}
        initialParams={{ nomeDoUsuario: nomeDoUsuario }}
      />
      <Drawer.Screen
        options={{
          headerShown: true,
          ...
          headerTitle: "",
          drawerLabel: ({ focused }) => (
            <Text style={styles.optionsTextos}>Sobre</Text>
          ),
          drawerIcon: ({ focused }) => (
            <Icon style={styles.optionsIcones} name="home" />
          ),
        }}
        name="TelaSobre"
        component={TelaSobre}
      ...
      <Drawer.Screen
        options={{
          ...
          headerTitle: "",
          drawerLabel: ({ focused }) => (
            <Text style={styles.optionsTextos}>Contatos</Text>
          ),
          drawerIcon: ({ focused }) => (
            <Icon style={styles.optionsIcones} name="contacts" />
          ),
        }}
        name="TelaContato"
      ...
      <Drawer.Screen
        options={{
          ...
          headerTitle: "",
          drawerLabel: ({ focused }) => (
            <Text style={styles.optionsTextos}>Câmpus</Text>
          ),
          drawerIcon: ({ focused }) => (
            <Icon style={styles.optionsIcones} name="school" />
          ),
        }}
        name="TelaCampus"
        component={TelaCampus}
      />
      <Drawer.Screen
        options={{
          headerShown: true,
          ...
          headerTitle: "",
          drawerLabel: ({ focused }) => (
            <Text style={styles.optionsTextos}>Conta</Text>
          ),
          drawerIcon: ({ focused }) => (
            <Icon style={styles.optionsIcones} name="settings" />
          ),
        }}
